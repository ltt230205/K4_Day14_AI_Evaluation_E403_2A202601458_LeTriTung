...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_recall": avg(context_recalls) if context_recalls else None,
            "avg_context_precision": avg(context_precisions) if context_precisions else None,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def metric_avg(results: list[EvalResult], metric: str) -> float:
            if not results:
                return 0.0
            return sum(float(getattr(result, metric)) for result in results) / len(results)

        metrics = ("faithfulness", "relevance", "completeness")
        new_avgs = {metric: metric_avg(new_results, metric) for metric in metrics}
        baseline_avgs = {
            metric: metric_avg(baseline_results, metric) for metric in metrics
        }
        regressions = [
            metric
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0) > 0:
            suggestions.append(
                "Add a grounding check that rejects claims not supported by retrieved context"
            )
        if categories.get("irrelevant", 0) > 0 or categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Clarify the agent prompt so answers directly address the user's support intent"
            )
        if categories.get("incomplete", 0) > 0:
            suggestions.append(
                "Improve retrieval coverage and require answers to include key policy conditions"
            )

        low_faithfulness = any(failure.faithfulness < 0.5 for failure in failures)
        low_relevance = any(failure.relevance < 0.5 for failure in failures)
        low_completeness = any(failure.completeness < 0.5 for failure in failures)

        if low_faithfulness and not any("retrieval" in s.lower() for s in suggestions):
            suggestions.append(
                "Review chunking and retrieval ranking for cases with unsupported answers"
            )
        if low_relevance and not any("prompt" in s.lower() for s in suggestions):
            suggestions.append(
                "Add few-shot examples that map common question intents to concise answers"
            )
        if low_completeness and not any("conditions" in s.lower() for s in suggestions):
            suggestions.append(
                "Add a completeness checklist for dates, exceptions, amounts, and required actions"
            )

        fallback_suggestions = [
            "Add the failed cases to the regression benchmark before changing prompts",
            "Inspect retrieved chunks for the worst cases and tune top-k or query wording",
            "Track failure type distribution on every benchmark run",
        ]
